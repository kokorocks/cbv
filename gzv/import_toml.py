"""Utilities to find files in parent directories and load TOML files.

This module provides a small, well-documented API and a CLI for
locating a TOML file by searching the current directory and its parents
and returning the parsed content.
"""
...
def find_file_in_parent_dirs(
    ...
    """Search for ``filename`` in ``start_path`` and its parent directories.
    ...
    Returns:
        A ``Path`` to the found file or ``None`` if not found.
    """
...
def load_toml_from_parent(filename: str = "config.toml", start_path: Union[str, Path, None] = None) -> Dict[str, Any]:
    """Find ``filename`` in ``start_path`` (or cwd) and parse it as TOML.
...
def _main(argv: list[str] | None = None) -> int:
    ...
    except Exception as exc:  # keep broad here for friendly CLI messaging
        print(f"Error: {exc}", file=sys.stderr)
        return 2

    # Use JSON for stable, readable printing (handles nested structures)
